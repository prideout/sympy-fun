...
def Sweep(curvePositions, curveNormals, crossSection):
    ...
    curveBasis = (curveBinormals.row_join(curveTangents).row_join(curveNormals))
...
r, R = symbols('r R')
positions = VVF(cos(u), sin(u), 0) # VVF(14*cos(12*t), -8*sin(-16*t), 0) # Lissajous http://www.waldomaths.com/Param1NL.jsp
normals = VVF(0, 0, 1)
crossSection = VVF(R + r*cos(v), 0, r*sin(v))
torus = Sweep(positions, normals, crossSection)
normals = NormalFunc(torus)

Print('Lissajous Surface', torus)
Print('Lissajous Normals', normals)
